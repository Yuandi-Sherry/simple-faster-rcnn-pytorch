...
def loc2bbox(src_bbox, loc):
    ...
    * :math:`\\hat{g}_w = p_w \\exp(t_w)`
    ...
    # src_bbox: (R, 4), R为bbox个数，todo 4为左下角和右上角四个坐标 还是左上和右下

    src_bbox = src_bbox.astype(src_bbox.dtype, copy=False)

    src_height = src_bbox[:, 2] - src_bbox[:, 0] # ymax-ymin
    src_width = src_bbox[:, 3] - src_bbox[:, 1] # ymax-ymin
    src_ctr_y = src_bbox[:, 0] + 0.5 * src_height # y0+0.5h
    src_ctr_x = src_bbox[:, 1] + 0.5 * src_width # x0 + 0.5w 计算中心点坐标
    # 从一维数组loc中分别取出每个y, x, h, w
    dy = loc[:, 0::4]
    dx = loc[:, 1::4]
    dh = loc[:, 2::4]
    dw = loc[:, 3::4]
# 边框回归，寻找原始proposal与近似目标框G之间的映射关系，这四行得到目标边框
...
def bbox2loc(src_bbox, dst_bbox): # 已知源框和目标框求其位置偏差
    ...
    * :math:`t_w = \\log(\\frac{g_w} {p_w})`
    ...
    eps = xp.finfo(height.dtype).eps # 最小正数
    height = xp.maximum(height, eps)
    width = xp.maximum(width, eps) # 全为非负
    # 真正的目标边框与proposal之间的偏移
    dy = (base_ctr_y - ctr_y) / height
    dx = (base_ctr_x - ctr_x) / width
    dh = xp.log(base_height / height)
    dw = xp.log(base_width / width) # 计算dx, dy, dh, dw

    loc = xp.vstack((dy, dx, dh, dw)).transpose() # np.vstack按行顺序把数组堆叠
    return loc
...
def bbox_iou(bbox_a, bbox_b):
    ...
    if bbox_a.shape[1] != 4 or bbox_b.shape[1] != 4:
        raise IndexError # 确保bbox第二维为bbox的四个坐标

    # tl为交叉部分框左上角坐标最大值，为了利用numpy的广播性质，bbox_a[:, None, :2]的shape是(N,1,2)，bbox_b[:, :2]shape是(K,2),由numpy的广播性质，两个数组shape都变成(N,K,2)，也就是对a里每个bbox都分别和b里的每个bbox求左上角点坐标最大值
...
def generate_anchor_base(base_size=16, ratios=[0.5, 1, 2],
    ...
    anchor_base = np.zeros((len(ratios) * len(anchor_scales), 4),
                           dtype=np.float32) # 以特征图的左上角位为基准产生9个anchor
